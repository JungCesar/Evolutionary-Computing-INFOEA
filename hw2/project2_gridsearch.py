...
def MLS(graph, num_starts, max_total_passes=None, time_limit=None):
    ...
    while i < effective_num_starts:
        ...
        if max_total_passes is not None:
            ...
            if total_passes_consumed >= max_total_passes: break
            fm_max_passes = int(remaining_passes) # Pass the remaining budget to FM

        try:
            # Run FM heuristic
            local_opt, passes_this_call = fm_heuristic(sol, graph, max_passes=fm_max_passes)
    ...
    if best_solution is None:
        if i > 0 : print("Warning: MLS completed without finding any valid solution.")
        else: print("Warning: MLS did not complete any starts due to immediate limit.")
        best_solution = generate_random_solution() # Return a random balanced solution as fallback
...
def ILS(graph, initial_solution, mutation_size, max_total_passes=None, time_limit=None):
    ...
    while True:
        ...
        try:
            local_opt, passes_this_call = fm_heuristic(mutated, graph, max_passes=fm_max_passes)
            ...
            if local_opt is not None:
                ...
                if local_opt.count(0) != len(local_opt)//2: # Skip if still imbalanced
...
def ILS_annealing(graph, initial_solution, mutation_size,
                  temperature=10.0, cooling_rate=0.98, min_temperature=0.1, max_no_improvement=20,
                  max_total_passes=None, time_limit=None):
    """
    ...
    while True:
        ...
        # --- Mutation ---
        mutated = mutate(current_solution, mutation_size) # Mutate the *current* solution
        if mutated.count(0) != len(mutated) // 2:
            mutated = balance_child(mutated)
        if mutated.count(0) != len(mutated) // 2:
        ...
        if max_total_passes is not None:
            remaining_passes = max(1, max_total_passes - total_passes_consumed)
            if total_passes_consumed >= max_total_passes: break # Check again
            fm_max_passes = int(remaining_passes)
        ...
        try:
            ...
            if local_opt is not None:
                ...
                if local_opt.count(0) != len(local_opt)//2:
                    print("Warning: ILS Annealing FM result imbalanced, skipping update.")
                ...
                if delta_e < 0:
                    accept = True
                     ...
                     except OverflowError: # math.exp can overflow for large delta_e/temp
                        accept = False
                ...
                if accept:
                    ...
                    if fit < best_fit:
                        best_solution = local_opt.copy()
                        best_fit = fit
                        no_improvement_on_best = 0 # Reset stagnation counter for *best*
                    else:
...
def ILS_adaptive(graph, initial_solution,
                 ...
                 min_mutation_size=1, max_mutation_size=None, # Allow setting max mutation
                 stagnation_window=15, stagnation_tolerance=0.0001, restart_reset=True, # Added params
                 max_total_passes=None, time_limit=None):
    """
    ...
    if current_solution.count(0) != len(current_solution) // 2:
        ...
        return (generate_random_solution(), float('inf'), [], 0, 0, int(initial_mutation_mean), 0)
    ...
    while iteration < effective_max_iterations:
        ...
        if local_opt_fit < current_fit:
            ...
            current_mutation_std_dev = max(1.0, min(current_mutation_mean * 0.5, current_mutation_std_dev * decay_factor))
        ...
        if use_max_iterations and stagnation_count >= stagnation_threshold:
            ...
            if len(fitness_history) >= stagnation_window:
                 ...
                 recent_diff = np.mean(np.abs(np.diff(fitness_history[-stagnation_window:])))
            ...
            if is_stuck:
                ...
                new_std_dev = min(current_mutation_mean / 2, current_mutation_std_dev * increase_factor)
                current_mutation_std_dev = max(1.0, new_std_dev) # Ensure std_dev is at least 1
...
def GLS(graph, population_size, stopping_crit=None, max_total_passes=None, time_limit=None):
    ...
    check_stagnation = (time_limit is None and max_total_passes is None and stopping_crit is not None)
    ...
    while pop_initialized_count < population_size:
        ...
        try:
            sol_opt, passes_this_call = fm_heuristic(initial_sol, graph, max_passes=fm_max_passes)
            total_passes_consumed += passes_this_call
            ...
            print(f"  Error during GLS init fm_heuristic (individual {pop_initialized_count+1}): {e}")
            # Continue trying to initialize the population

    init_end_time = time.perf_counter()
    print(f"GLS: Initialization finished ({pop_initialized_count}/{population_size} individuals). Passes: {total_passes_consumed}, Time: {init_end_time - init_start_time:.2f}s")
    ...
    while gen < effective_max_generations:
        ...
        if child.count(0) != len(child)//2: child = balance_child(child) # Ensure child is balanced
...
def run_grid_search(graph, grid_search_pass_limit, grid_search_runs):
    ...
    print(f"(Using Pass Limit: {grid_search_pass_limit}, Runs per Combo: {grid_search_runs})")
    ...
    adaptive_param_grid = {
        ...
        'mutation_std_dev': [2.0, 5.0, 10.0, 20.0], # Switched to absolute std dev, wider range
    ...
    for combo_idx, combo in enumerate(adaptive_combinations):
        ...
        display_params = {k: f'{v:.2f}' if isinstance(v, float) else v for k, v in params.items() if k in adaptive_keys}
        print(f"  ({combo_idx+1}/{len(adaptive_combinations)}) Testing Adaptive Params: { display_params }")
        ...
        for r in range(grid_search_runs):
            ...
            res_tuple = ILS_adaptive(graph, initial_sol,
                                     max_total_passes=grid_search_pass_limit,
                                     time_limit=None, # No time limit for grid search
                                     **params) # Pass parameters using dictionary unpacking
    ...
    for combo_idx, combo in enumerate(annealing_combinations):
        ...
        combo_fitnesses = []
        display_params = {k: f'{v:.3f}' if isinstance(v, float) else v for k, v in params.items()} # Show more precision for cooling
        print(f"  ({combo_idx+1}/{len(annealing_combinations)}) Testing Annealing Params: { display_params }")
...
def run_MLS_experiment(graph):
    ...
    for i in range(EXPERIMENT_RUNS):
        ...
        results.append((i, solution_str, fit, comp_time, passes_consumed, "", "", "", "")) # Keep structure consistent
...
def run_simple_ILS_experiment(graph):
    ...
    for mutation_size in mutation_sizes:
        ...
        for i in range(EXPERIMENT_RUNS):
            ...
            results.append((i, solution_str, fit, comp_time, passes_consumed, "Mutation_Size", mutation_size, "Unchanged_Count", unchanged_count))
...
def run_GLS_experiment(graph):
    ...
    for i in range(EXPERIMENT_RUNS):
        ...
        solution_str = "".join(map(str, best_sol)) if best_sol else ""
        # Results tuple: (run_idx, solution, fitness, time, passes, param1_name, param1_val, param2_name, param2_val)
        results.append((i, solution_str, fit, comp_time, passes_consumed, "Stopping_Crit", stopping_crit, "", ""))
        print(f"       Run {i+1}: Fitness={fit}, Passes={passes_consumed}, Time={comp_time:.4f}s")
    return results
...
def run_ILS_adaptive_experiment(graph, best_params):
    ...
    results = []
    if not best_params:
        print("   WARNING: No best parameters found for Adaptive ILS. Skipping experiment.")
        return results
    ...
    for i in range(EXPERIMENT_RUNS):
        ...
        res_tuple = ILS_adaptive(
            graph, initial_sol, max_total_passes=FM_PASSES_LIMIT, time_limit=None, **exp_params
        )
        end_time = time.perf_counter()
        comp_time = end_time - start_time
        ...
        results.append((i, solution_str, fit, comp_time, passes_consumed, "Final_MutMean", final_mut_mean, "Final_Stagnation", final_stag_count))
...
def run_ILS_annealing_experiment(graph, best_params):
    ...
    results = []
    if not best_params:
        print("   WARNING: No best parameters found for Annealing ILS. Skipping experiment.")
        return results
    # Create a clean copy for the experiment run
    exp_params = best_params.copy()
    print(f"   Using Parameters: {exp_params}")
    mutation_size = exp_params.get('mutation_size', 50) # Extract mutation size for logging if needed
...
def run_ILS_runtime_experiment(graph, time_limit, mutation_size):
    ...
    # Return tuple: (run_idx, solution, fitness, actual_duration, passes, ...)
    return [(0, solution_str, fit, duration, passes_consumed, "Mutation_Size", mutation_size, "Unchanged_Count", unchanged_count)]
...
def run_GLS_runtime_experiment(graph, time_limit, stopping_crit):
    ...
    # Return tuple: (run_idx, solution, fitness, actual_duration, passes, ...)
    return [(0, solution_str, fit, duration, passes_consumed, "Stopping_Crit", stopping_crit, "", "")]
...
def run_ILS_adaptive_runtime_experiment(graph, time_limit, best_params):
    ...
    if not best_params:
        print("   WARNING: No best parameters for Adaptive ILS runtime. Using defaults.")
    ...
    res_tuple = ILS_adaptive(graph, initial_sol, time_limit=time_limit, max_total_passes=None, **exp_params)
    end_run_time = time.perf_counter() # End timer
    duration = end_run_time - start_run_time # Calculate actual duration
    ...
    # Return tuple: (run_idx, solution, fitness, actual_duration, passes, ...)
    return [(0, solution_str, fit, duration, passes_consumed, "Final_MutMean", final_mut_mean, "Final_Stagnation", final_stag_count)]
...
def run_ILS_annealing_runtime_experiment(graph, time_limit, best_params):
    ...
    if not best_params:
        print("   WARNING: No best parameters for Annealing ILS runtime. Using defaults.")
        # Fallback to some reasonable defaults
        best_params = {
            'mutation_size': 50, 'temperature': 10.0, 'cooling_rate': 0.98,
            'min_temperature': 0.1, 'max_no_improvement': 20
        }
    exp_params = best_params.copy() # Use a copy
    ...
    best_sol, unchanged_count, passes_consumed = ILS_annealing(graph, initial_sol, time_limit=time_limit, max_total_passes=None, **exp_params)
    end_run_time = time.perf_counter() # End timer
    duration = end_run_time - start_run_time # Calculate actual duration
...
def save_intermediate_results(results_list, script_dir, filename="experiment_results_intermediate.csv"):
    ...
    final_columns = [
        ...
        "Fitness", "Comp_Time", "Actual_Passes",
        "Solution"
    ]
    ...
    fill_values = {
        "Param1_Name": "", "Param1_Value": pd.NA, "Param2_Name": "", "Param2_Value": pd.NA,
        "Actual_Passes": -1, "Fitness": float('inf'), "Comp_Time": -1.0, "Solution": ""
    }
    for col, val in fill_values.items():
        if col in df_intermediate.columns: df_intermediate[col].fillna(val, inplace=True)
        else: df_intermediate[col] = val

    # Convert types
    df_intermediate['Run'] = pd.to_numeric(df_intermediate['Run'], errors='coerce').astype('Int64')
    df_intermediate['Fitness'] = pd.to_numeric(df_intermediate['Fitness'], errors='coerce')
    df_intermediate['Comp_Time'] = pd.to_numeric(df_intermediate['Comp_Time'], errors='coerce')
    df_intermediate['Actual_Passes'] = pd.to_numeric(df_intermediate['Actual_Passes'], errors='coerce').astype('Int64')
    df_intermediate['Param1_Value'] = pd.to_numeric(df_intermediate['Param1_Value'], errors='ignore')
    df_intermediate['Param2_Value'] = pd.to_numeric(df_intermediate['Param2_Value'], errors='ignore')
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not os.path.exists(graph_file_path):
            ...
            if not os.path.exists(graph_file_path):
                 raise FileNotFoundError(f"Graph file 'Graph500.txt' not found in script directory or CWD.")
    ...
    try:
        best_adaptive_params, best_annealing_params = run_grid_search(graph, GRID_SEARCH_PASS_LIMIT, GRID_SEARCH_RUNS)
        ...
        if os.path.exists(params_path):
            try:
                print(f"Attempting to load previously saved parameters from {params_path}")
                ...
                print(f"Failed to load parameters: {load_e}. ILS Adaptive/Annealing may use defaults.")
        else:
    ...
    def append_and_save(new_results, experiment_name):
        ...
        for r in new_results:
            ...
            if len(r) > 5: result_dict["Param1_Name"] = r[5]
            if len(r) > 6: result_dict["Param1_Value"] = r[6]
            if len(r) > 7: result_dict["Param2_Name"] = r[7]
            if len(r) > 8: result_dict["Param2_Value"] = r[8]
            all_results_list.append(result_dict)
        # Save intermediate results after appending
        save_intermediate_results(all_results_list, script_dir, intermediate_results_file)
    # -----------------------------
    ...
    for r in mls_results_pass: append_and_save([r], "MLS_PassBased")

    # Run Simple ILS
    simple_ils_results_pass = run_simple_ILS_experiment(graph)
    for r in simple_ils_results_pass: append_and_save([r], "Simple_ILS_PassBased")

    # Run GLS
    gls_results_pass = run_GLS_experiment(graph)
    for r in gls_results_pass: append_and_save([r], "GLS_PassBased")
    ...
    for r in ils_adaptive_results_pass: append_and_save([r], "ILS_Adaptive_PassBased")

    # Run ILS Annealing with BEST parameters found
    ils_annealing_results_pass = run_ILS_annealing_experiment(graph, best_annealing_params)
    for r in ils_annealing_results_pass: append_and_save([r], "ILS_Annealing_PassBased")
    ...
    try:
        ...
        if not df_simple_ils.empty and 'Param1_Value' in df_simple_ils.columns and 'Fitness' in df_simple_ils.columns:
            # Ensure types are numeric for aggregation
            df_simple_ils['Param1_Value'] = pd.to_numeric(df_simple_ils['Param1_Value'], errors='coerce')
            ...
            if not df_simple_ils.empty:
                ...
                if not median_fits.empty:
                    ...
                    print(f"Best Simple ILS Mutation Size from Pass-Based runs: {best_ils_mutation_size} (Median Fitness: {median_fits.min():.2f})")
                else: print("Could not calculate median fitness for Simple ILS.")
            else: print("No valid numeric data for Simple ILS parameter determination.")
        else: print("Simple ILS results missing required columns or empty.")
    except Exception as e:
        print(f"Could not determine best Simple ILS param: {e}. Using default: {best_ils_mutation_size}")
    ...
    for i in range(RUNTIME_RUNS):
        ...
        print(f"    Simple ILS Done: Fitness={ils_s_run_res[0][2]}, Passes={ils_s_run_res[0][4]}")
        ...
        ils_ad_run_res = run_ILS_adaptive_runtime_experiment(graph, DYNAMIC_TIME_LIMIT, best_adaptive_params)
        append_and_save([ils_ad_run_res[0]], "ILS_Adaptive_Runtime")
        print(f"    Adaptive ILS Done: Fitness={ils_ad_run_res[0][2]}, Passes={ils_ad_run_res[0][4]}")

        # Annealing ILS Runtime (using best params from grid search)
        print("  Running Annealing ILS Runtime (Best Params)...")
        ils_a_run_res = run_ILS_annealing_runtime_experiment(graph, DYNAMIC_TIME_LIMIT, best_annealing_params)
        append_and_save([ils_a_run_res[0]], "ILS_Annealing_Runtime")
        print(f"    Annealing ILS Done: Fitness={ils_a_run_res[0][2]}, Passes={ils_a_run_res[0][4]}")
    ...
    final_columns = [
        "Experiment", "Run",
        "Param1_Name", "Param1_Value", "Param2_Name", "Param2_Value", # Generic parameter columns
        "Fitness", "Comp_Time", "Actual_Passes",
    ...
    fill_values = {
        "Param1_Name": "", "Param1_Value": pd.NA, # Use pd.NA for missing numeric/object
        "Param2_Name": "", "Param2_Value": pd.NA,
    ...
    df_experiments['Run'] = pd.to_numeric(df_experiments['Run'], errors='coerce').astype('Int64')
    df_experiments['Fitness'] = pd.to_numeric(df_experiments['Fitness'], errors='coerce')
    df_experiments['Comp_Time'] = pd.to_numeric(df_experiments['Comp_Time'], errors='coerce')
    df_experiments['Actual_Passes'] = pd.to_numeric(df_experiments['Actual_Passes'], errors='coerce').astype('Int64')
    # Try converting Param values, but ignore errors as they can be strings or numbers
    df_experiments['Param1_Value'] = pd.to_numeric(df_experiments['Param1_Value'], errors='ignore')
    df_experiments['Param2_Value'] = pd.to_numeric(df_experiments['Param2_Value'], errors='ignore')

    # Define output path relative to script directory
    output_csv_path = os.path.join(script_dir, "experiment_results_combined_gridsearch_runtimefix.csv")
    try:
